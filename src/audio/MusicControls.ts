...
export class MusicControls {
  ...
  private readonly cleanup:AbortController=new AbortController();

  constructor(private readonly panel:HTMLElement){
    const element=<T extends HTMLElement>(id:string)=>panel.querySelector<T>(`#${id}`)!;
    this.play=element('music-play');this.title=element('music-title');this.message=element('music-message');
    this.seek=element('music-seek');this.time=element('music-time');this.volume=element('music-volume');
    this.file=element('music-file');this.detail=element('music-options');this.audio=element('music-audio');
    ...
    element('music-demo').addEventListener('click',()=>{this.detail.open=false;void this.player.included();},options);
    element('music-tab').addEventListener('click',()=>{void this.player.shareTab().then(()=>{if(this.player.state.kind==='tab')this.detail.open=false;});},options);
    this.seek.addEventListener('input',()=>{
      if(Number.isFinite(this.audio.duration)&&this.audio.duration>0){
        this.audio.currentTime=Number(this.seek.value);this.renderProgress();
      }
    },options);
    ...
    window.addEventListener('drop',event=>{
      ...
      event.preventDefault();void this.player.choose(event.dataTransfer.files[0]);
    },options);
    this.render();
  }
  ...
  private readonly renderProgress=():void=>{
    ...
    this.seek.max=String(duration||1);this.seek.value=String(current);
